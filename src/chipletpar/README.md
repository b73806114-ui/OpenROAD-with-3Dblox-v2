...
When this repository is checked out as `OpenROAD/src/chipletpar`, OpenROAD's
CMake build creates `chipletpar_core` and links the `chipletpar` adapter into
the main executable.  The adapter uses OpenROAD's own 3DBlox reader and writer,
annotates the resulting OpenDB `dbChipInst` objects, and preserves the
ChipletPart vendor data in the output 3DBlox files.

The primary OpenROAD flow operates on the current in-memory OpenDB:

```tcl
read_3dbx /path/to/design.3dbx
chipletpar_partition -config /path/to/design.3dbx -seed 42
write_db /path/to/partitioned.odb
chipletpar_write_3dblox -output /path/to/partitioned_3dblox
```

`-config` imports the ChipletPart cost-model dataset from the source `.3dbx`
and persists it as a top-chip OpenDB property. The partition IDs, summary, and
cost metadata are therefore preserved by `write_db`. After reloading that
database, neither the original 3DBlox file nor `-config` is required:

```tcl
read_db /path/to/partitioned.odb
chipletpar_partition -seed 42
puts [chipletpar_report]
write_db /path/to/repartitioned.odb
```

`chipletpar_clear` removes the partition result while retaining the persisted
cost metadata. `chipletpar_write_3dblox` writes the `.3dbx`, referenced
`.3dbv`, Verilog, DEF, bump-map, and technology files, and copies the database
partition result into `chipletpart_vendor_data`.

For compatibility, the original all-in-one command remains available:

```tcl
chipletpar_partition_3dblox \
  -input /path/to/input_design \
  -output /path/to/partitioned_design \
  -seed 42
```

Its input directory must contain exactly one `.3dbx` entry file and the
referenced `.3dbv`/collateral files. The output directory is created when it
does not exist, and the output `.3dbx` keeps the input filename. Passing direct
`.3dbx` paths remains supported.

By default this preserves the original ChipletPart behavior: it searches
candidate counts from 1 through 8, applies the original refinement and cost
model, and emits the lowest-cost result. `-max_parts` optionally changes the
search upper bound. For a single fixed METIS run, specify `-num_parts`; the two
options are mutually exclusive. The original cost-model inputs retain the
standalone wrapper defaults and can be overridden with `-reach` (0.5),
`-separation` (0.25), and `-technology` (`7nm`).

The partition commands return a Tcl dictionary containing `vertices`,
`partitions`, `edge_cut`, `cost`, `mode`, `annotated_odb_instances`, and
`missing_odb_instances`. The compatibility command additionally returns
`output`. The in-memory command currently expects a chiplet-level database
containing `dbChipInst` objects; ordinary flat `dbInst/dbNet` databases require
an explicit grouping policy and are not yet accepted.
