                  ...
                  const std::vector<Net>& nets)
{
  ...
  using SearchNode = std::pair<Solution, std::vector<std::uint16_t>>;
  ...
  while (!stack.empty()) {
    ...
    if (!first_node
        && std::find(feasible_successors.begin(),
                     feasible_successors.end(),
                     current.last_net)
               == feasible_successors.end()) {
      first_node = false;
      continue;
    ...
    if (current.order.size() == net_count) {
      ...
      for (std::size_t index = 0U; index < current_remaining.size(); ++index) {
        Solution child = current;
        std::vector<std::uint16_t> child_remaining = current_remaining;
        ...
        stack.emplace_back(std::move(child), std::move(child_remaining));
      }
    }
  ...
  std::cout << "\nNode count: " << static_cast<unsigned long>(node_count)
            << std::endl;
