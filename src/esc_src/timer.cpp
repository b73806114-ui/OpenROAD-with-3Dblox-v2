...
Timer::Timer(const std::string& function_name)
    : function_name_(function_name), start_(std::chrono::system_clock::now())
{
}
...
Timer::~Timer()
{
  ...
  constexpr const char* separator
      = "====================================================================";
  std::cout << '\n' << separator << '\n';
  std::cout << std::left << std::setw(30) << "|  Function Name" << std::right
            << std::setw(15) << " Exec Count" << std::setw(23)
            << " Exec Time  |" << '\n';
  std::cout << separator << '\n';
