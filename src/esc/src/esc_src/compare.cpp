...
std::vector<std::uint16_t> normalized_profile(const Escaper& escaper,
                                              const Solution& solution)
{
  ...
  if (column_count == 0U || solution.contour.size() < column_count) {
    return profile;
  }
                            ...
                            const std::vector<std::uint16_t>& rhs)
{
  return std::equal(
      ...
      rhs.begin(),
      rhs.end(),
      [](std::uint16_t left, std::uint16_t right) { return left <= right; });
}
...
bool Escaper::compare(const Solution& candidate,
                      Solution& incumbent,
