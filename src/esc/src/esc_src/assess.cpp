...
bool Escaper::assess(const Solution& candidate, unsigned int count,
    ...
    for (unsigned int worker = 0U; worker != 8U; ++worker) {
        ...
        boost::asio::post(pool, [this, &candidate, &incumbents,
                                 compare_contours, begin, end]() {
