...
double polyline_length(const std::vector<Point>& path)
{
  ...
  for (std::size_t index = 1U; index < path.size(); ++index) {
    const double delta_x = path[index].first - path[index - 1U].first;
    const double delta_y = path[index].second - path[index - 1U].second;
    length = length + std::sqrt(delta_x * delta_x + delta_y * delta_y);
  }
  return length;
}
...
double interpolated_y(const std::vector<Point>& path, std::size_t index)
{
  ...
  return ((next.second - previous.second) * (current.first - previous.first))
                                           ...
                                           std::vector<Net>& nets)
{
  ...
  for (const std::uint32_t net : optimal_order) {
    ...
    for (std::size_t column = 0U; column < replay.contour.size(); ++column) {
  ...
  for (std::size_t column = 0U; column < resource_table.size(); ++column) {
    ...
    while (route_index < routes.size()) {
      ...
      Point& first_point = routes[run_begin][column - first_columns[run_begin]];
      ...
      while (run_end < routes.size()
             ...
             && same_point(first_point,
                           routes[run_end][column - first_columns[run_end]])) {
        ++run_end;
      }
      ...
      } else if (interval_index != resource_table[column].size() - 1U) {
        const std::vector<Point>& samples
            = resource_table[column][interval_index].points;
        if (samples.size() == 1U) {
          top_y = top_y + static_cast<double>(run_size >> 1U) * pitch;
      ...
      for (std::size_t index = run_begin; index < run_end; ++index) {
        Point& point = routes[index][column - first_columns[index]];
  ...
  for (std::uint32_t pass = 0U; pass < 10U; ++pass) {
    ...
    for (std::size_t route_index = 0U; route_index < routes.size();
      ...
      for (std::size_t point_index = 0U; point_index < source.size();
           ++point_index) {
        if (point_index == 0U || point_index + 1U == source.size()) {
        ...
        const double limit = limits[first_column + point_index];
        double selected_y;
      ...
      for (std::size_t point_index = 0U; point_index < adjusted.size();
           ++point_index) {
        const bool endpoint
            = point_index == 0U || point_index + 1U == adjusted.size();
        limits[first_column + point_index]
            = adjusted[point_index].second
              - (endpoint ? BUMP_SIZE * 0.5 + NET_WIDTH * 0.5 + MIN_SPACING
                          : pitch);
      }
    }
